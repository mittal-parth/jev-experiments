...
- `--lead-frames 8` (default) is how many runner frames before a standing collision we jump, if the hop still clears. **Lower** (4–6) jumps later — better on clustered cacti, easier to clip the first one. **Higher** (10–14) jumps earlier — safer on singles, lands on wide clusters. Same constant in [`dino_jev/physics.py`](dino_jev/physics.py) and [`dino_jev/static/bot.js`](dino_jev/static/bot.js). No in-game sliders.
